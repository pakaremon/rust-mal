  ...
  def __loadReleases(self) -> None:
    ...
    for release in self.package_data["releases"][self.version]:
      if "url" in release:
        ...
        package_type = release.get("packagetype", None)
        if release_url:
            self.releases.append(PyPackageRelease(self, release_url, package_type))
...
class PyPackageRelease():
  ...
  def __init__(self, pypackage:PyPackage ,url:str, package_type:str = None) -> None:
    """
      Constructor of the PyPackageRelease class

        Parameters:
          pypackage(PyPackage): The package owner of this release
