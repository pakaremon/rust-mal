...
class Utils():
  ...
  def rmtree(folder_path):
    """
      Custom reimplementation of shutil.rmtree that works under Windows.
      (shutil.rmtree raises exceptions under Windows if permissions are not right)
    ...
    def onerror(func, path, exc_info):
        ...
        try:
            ...
            print(f"Failed to handle error for {path}: {e}")
